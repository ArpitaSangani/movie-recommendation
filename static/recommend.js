$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '8f1244df6398593cab5fa022446db4cf';
    var title = $('.movie').val();
    if (title=="") {
...
function show_details(movie_details,arr,movie_title,my_api_key,movie_id){
  ...
  var tmdb_reviews = get_tmdb_reviews(movie_id, my_api_key);
  ...
  var details = {
    ...
    'tmdb_reviews':JSON.stringify(tmdb_reviews),
  }
...
function get_tmdb_reviews(movie_id, my_api_key) {
  ...
  $.ajax({
    type: 'GET',
    url: 'https://api.themoviedb.org/3/movie/' + movie_id + '/reviews?api_key=' + my_api_key,
    async: false,
    success: function(data) {
      for (var i = 0; i < Math.min(data.results.length, 10); i++) {
        reviews[data.results[i].content] = 'Good';
      }
    }
  });
  return reviews;
